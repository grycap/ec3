...
def get_queued_jobs():
    try:
        ...
        return {"wn": 0}
    r = {}
    for job in root.iter("Job") if hasattr(root, "iter") else root.getiterator("Job"):
        if job.find("job_state").text.strip().upper()[0] == "Q":
            q = job.find("queue").text.strip()
            v = int(job.find("Resource_List").find("nodes").text)
            r[q] = r.get(q, 0) + v
    return r
